...
void mov(operand_t params[]) {
    if(params[1].type == REGISTER && params[2].type == REGISTER) {
        printf("I came to here!");
        if(params[1].reg.bytes == params[2].reg.bytes )
            setRegister(params[1].reg,params[2].value);
        else {
            printf("Incompatible registers");
            return;
        }
    } else if(params[2].type == NUMBER) {
        setRegister(params[1].reg,params[2].value);
        int h = 0;
        getRegister(params[1].reg,&h);
        printf("%d",h);
    }
}
